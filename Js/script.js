  ...
  .then(svgText => {
    ...
    setTimeout(() => {
      allPaths.forEach((path, i) => {
       path.style.transition = `stroke-dashoffset 0.1s cubic-bezier(.77,0,.18,1) ${i * 0.02}s, fill-opacity 0.35s ${0.4 + i * 0.02}s`;
    ...
    const totalDuration = 500 + (allPaths.length - 1) * 20 + 200;
...
function showDedicationText() {
  ...
  if (!text) {
    text = `Un viaje contigo...  
cada kilómetro, cada canción, cada silencio y cada risa,  
me hacen sentir que el destino me llevo a tomar el lugar correcto. 🚌❤️
Prometo no molestar mucho, pero tenía que decir que tu sonrisa cambió por completo el ambiente. 
Te vi por un instante y ya quise que el viaje durara toda la vida; 
dicen que los trayectos están llenos de historias, y la mía empezó cuando te vi.
No importa a dónde vayamos,  
mientras el asiento a tu lado siga disponible para mí.`;  } else {
...
function showSignature() {
  ...
  signature.textContent = firma ? decodeURIComponent(firma) : "Con cariño,Omar";
